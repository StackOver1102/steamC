...
const ProductCarousel = ({ game }) => {
  ...
  useEffect(() => {
    const images = game[0].images.map((image) => ({
      url: image,
      type: "image",
    }));

    const videos = game[0].videos.map((video) => ({
      url: video,
      type: "video",
    }));
  ...
  return (
    <div className="w-[940px] mx-auto mb-[100px]">
          ...
          <div id="right-col" className="w-[324px] ml-0 float-right">
            <div id="glance_ctn" className="p-0">
              <div id="game-image" className="mb-[7px]">
                <img src={game[0].game_image} className="w-[324px] h-[151px]" />
              </div>
              <div
                id="game-description"
                style={{
      ...
      <div
        className="w-[940px] my-0 mx-auto mb-[16px]"
        style={{ background: " rgba(0, 0, 0, 0.2)" }}
      >
        <div className="p-4">
          <p className="text-white">
            <Link to="/login" className="text-cyan-500">Đăng nhập </Link>
            để thêm sản phẩm này vào danh sách ước, theo dõi, hoặc đánh dấu nó
            thành "đã phớt lờ"
          </p>
        </div>
      </div>
